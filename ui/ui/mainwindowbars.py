...
from .shared import TITLEBAR_HEIGHT, WINDOW_BORDER_WIDTH, BOTTOMBAR_HEIGHT, LEFTBAR_WIDTH, LEFTBTN_WIDTH
from .framelesswindow import FramelessMoveResize
from .ui_config import pcfg, save_config
from . import shared as C
...
class LeftBar(Widget):
    ...
    def __init__(self, mainwindow, *args, **kwargs) -> None:
        ...
        actionOpenProj = QAction(self.tr("Open Project (*.json *.Proj)"), self)
        ...
        exportResult = QAction(self.tr("Export Results"), self)
        self.export_result = exportResult.triggered

        self.recentMenu = QMenu(self.tr("Open Recent"), self)
        
        openMenu = QMenu(self)
        openMenu.addActions([actionOpenProj])
        ...
        # self.rectTool = ToolCheckBox()
        # self.rectTool.setObjectName("RectTool")

        vlayout = QVBoxLayout(self)
        vlayout.addWidget(openBtnToolBar)
        vlayout.addWidget(self.showPageListLabel)
        # vlayout.addWidget(self.rectTool)
    ...
    def onOpenProj(self) -> None:
        ...
        selected_file = ''
        dialog = QFileDialog()
        selected_file = str(dialog.getOpenFileUrl(self.parent(), self.tr('Open Project file *.json/*.txt'), filter="*.json *.txt")[0].toLocalFile())
        if not osp.exists(selected_file):
            return
        if osp.exists(selected_file):
            self.open_proj.emit(selected_file)
...
class TitleBar(Widget):
    ...
    def __init__(self, parent, proj, *args, **kwargs) -> None:
        super().__init__(parent, *args, **kwargs)
        self.proj: ProjSeg = proj
        ...
        editMenu.addActions([pageSearchAction, undoAction, redoAction])
        ...
        self.titleLabel = QLabel('SketchSeg')
    ...
    def setTitleContent(self, proj_name: str = None, page_name: str = None, save_state: str = None):
        ...
        max_page_len = 50
        progress = ''
        if len(self.proj.pages) > 0:
            progress = f'{self.proj.current_idx + 1} / {len(self.proj.pages)} '
...
class BottomBar(Widget):
    ...
    def __init__(self, mainwindow: QMainWindow, *args, **kwargs) -> None:
        ...
        # self.scaleSlider = PaintQSlider(self.tr("Scale"), Qt.Orientation.Horizontal, self)
        # self.scaleSlider.setFixedWidth(150)
        ...
        self.scaleEditor = LineEdit(validator_type=int, validator_range=[1, 1000], suffix='%', init_value=100)
        self.hlayout.addWidget(self.scaleEditor)
        self.scaleEditor.setFixedWidth(50)
        self.hlayout.addStretch(-1)

        self.progress_bar = TaskProgressBar(self.tr('Progress'), verbose=True, flatten=True)
