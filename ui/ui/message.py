...
class MessageBox(QMessageBox):
    ...
    def __init__(self, info_msg: str = None, btn_type = QMessageBox.StandardButton.Ok, frame_less: bool = False, modal: bool = False, signal_slot_map_list: List[Dict] = None, info_type='', *args, **kwargs):
        super().__init__(text=info_msg, *args, **kwargs)
        self.register_signal_slot_map = []
        shared.add_to_runtime_widget_set(self)
    ...
    def clear_before_close(self):
        shared.remove_from_runtime_widget_set(self)
        self.disconnect_all()
...
class TaskProgressBar(Widget):
    def __init__(self, description: str = '', verbose=False, flatten=False, *args, **kwargs) -> None:
