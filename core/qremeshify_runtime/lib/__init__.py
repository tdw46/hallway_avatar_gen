...
class Quadwild():
    def __init__(self, mesh_path: str) -> None:
        ...
        runtime_dir = str(paths.qremeshify_runtime_dir())
        quadwild_lib_path = path.join(runtime_dir, quadwild_lib_filename)
        quadpatches_lib_path = path.join(runtime_dir, quadpatches_lib_filename)
    ...
    def quadrangulate(
        ...
        runtime_dir = str(paths.qremeshify_runtime_dir())
        params.flow_config_filename = path.join(runtime_dir, flow_config_files[flowConfig]).encode()
        params.satsuma_config_filename = path.join(runtime_dir, satsuma_config_files[satsumaConfig]).encode()
